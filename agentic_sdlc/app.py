...
# ── Global State (Karakterler Yeni Sisteme Eşlendi) ─────────
app_state = {
    "status": "idle",
    "project_name": "",
    ...
    "tasks": [],
    "agents": {
        "orchestrator": {"status": "waiting", "tasks_handled": 0, "output": "single_agent_result.txt"},
        "planner":      {"status": "waiting", "tasks_handled": 0, "output": "hypotheses.json"},
        "analyst":      {"status": "waiting", "tasks_handled": 0, "output": "prosecutor_attack.txt"},
        "engineer":     {"status": "waiting", "tasks_handled": 0, "output": "lawyer_defense.txt"},
        "validator":    {"status": "waiting", "tasks_handled": 0, "output": "final_verdict.txt"},
    },
...
def set_agent_status(agent_key: str, status: str):
    if agent_key in app_state["agents"]:
...
def run_pipeline_thread(project_name: str, customer_wish: str):
    try:
        ...
        push_log(f"❌ Sistem Hatası: {str(e)}")
...
def agent_page(key):
    if key not in app_state["agents"]:
        return "Ajan bulunamadı", 404
    
    # Arayüzdeki kutuların isimleri
    agent_names = {
        "orchestrator": "Tekil Ajan (Baseline)",
        "planner": "Baş Dedektif",
        "analyst": "Savcı",
        "engineer": "Savunma Avukatı",
        "validator": "Yargıç"
    }
...
def api_start():
    if app_state["status"] == "running":
        return jsonify({"error": "Sistem zaten çalışıyor."}), 400
    data = request.json
    project_name = data.get("project_name", "").strip()
    customer_wish = data.get("customer_wish", "").strip()
    if not project_name or not customer_wish:
        return jsonify({"error": "Proje adı veya vaka eksik."}), 400
